...
export function buildDashboardData(): DashboardData {
  ...
  const weekAgoStr = formatInTimeZone(addDays(now, -7), timezone, "yyyy-MM-dd'T'00:00:00");
  ...
  const allFamily = listFamilyMembers(userId);
  ...
  const activeReminders = reminders.filter((r) => !r.is_completed && !r.notified);

  // Tasks due within 2 weeks
  const tasks = getTasksDueInRange(todayStart, twoWeeksOut);
  const overdueTasks = tasks.filter((t) => t.due_at < now.toISOString());

  // Shopping
  const shoppingItems = listShoppingItems(userId);

  // Meals cooked this week
  const mealHistory = getMealHistory(userId, 30);
  const weekAgo = addDays(now, -7);
  const mealsThisWeek = mealHistory.filter(
    (m) => new Date(m.cooked_at + "Z") >= weekAgo
  );

  // School news
  const schoolEmails = listRecentSchoolEmails(5);

  // Usage stats
  const usageByDay = getMessageCountsByDay(7);

  // Events (next 7 days)
  const tomorrowStart = formatInTimeZone(now, timezone, "yyyy-MM-dd'T'00:00:00");
  const events = listEvents(userId, { fromDate: tomorrowStart, toDate: weekAheadStr });
  ...
  const todayMD = formatInTimeZone(now, timezone, "MM-dd");
  ...
  for (const member of allFamily) {
    ...
    if (daysUntil <= 30) {
      ...
      const turningAge = birthYear > 1900 ? thisYear - birthYear + (daysUntil === 0 ? 0 : birthdayThisYear.getFullYear() - thisYear) : null;
      const age = turningAge !== null ? (birthdayThisYear.getFullYear() - birthYear) : null;
  ...
  return {
    ...
    remindersToday: activeReminders.map((r) => ({
      title: r.title,
      dueAt: formatInTimeZone(new Date(r.due_at + "Z"), timezone, "h:mm a"),
    ...
    tasksDueSoon: tasks.slice(0, 10).map((t) => ({
      title: t.title,
      dueAt: formatInTimeZone(new Date(t.due_at + "Z"), timezone, "EEE d MMM"),
    ...
    nextGames: todayFixtures.map((f) => ({
      ...
      startTime: formatInTimeZone(new Date(f.start_time), timezone, "h:mm a"),
      venue: f.venue,
